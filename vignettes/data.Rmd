...
**Pre-processed data** are obtained by filling missing dates in the raw data with `NA` values. This ensures that all locations share the same grid of dates and no single day is skipped. Then, `NA` values are replaced with the previous non-`NA` value or `0`. Due to the incresing size of the data files, we stopped providing the pre-processed data on **01 April 2021**, so to improve the update and storage of the raw data. **Please switch to the raw data if you are still using the pre-processed files.**
...
The datasets include the number of people who have been **vaccinated** starting from 2021-01-02. 

The **datasets** are updated on an hourly basis. The workflow takes around one hour to process the data worldwide and the release takes place only when all the datasets are ready. This means there is usually between 1h and 2h lag with respect to the data providers. 

The **vintage data** are generated with a delay of 48 hours to make sure all the observations are complete, and we don't take snapshots of yet-not-complete data. Infact, there is a natural delay in reporting the data to the local authorities (+24h) and different time zones worldwide (+24h).
